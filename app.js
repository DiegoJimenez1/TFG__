...
app.get('/',(req,res) => {
    ...
    app.post('/GastoConjunto', (req, res) => {
        ...
        conexion.query(query, (error, results) => {
          ...
          var IDGrupoGasto = req.body.IDGrupoGasto ;
      
          console.log("valor json")
          console.log(req.body)
          var sql_aux = `INSERT INTO Gasto_Conjunto(ID_group, Name, ID_user, Fecha_creacion, Description, Gasto_total, Moneda, Estado, Fecha_cierre, Fecha_eliminacion, IDGrupoGasto) 
            VALUES ("${ID_group}", "${Name}", "${ID_user}", "${Fecha_creacion}", "${Description}", "${Gasto_total}", "${Moneda}", "${Estado}", "${Fecha_cierre}", "${Fecha_eliminacion}", "${IDGrupoGasto}")`;
          ...
          conexion.query(sql_aux, (error, result) => {
            ...
            if (Users && Users.length) {
                for (let i = 0; i < Users.length; i++) {
                    ...
                    const Contribucion = ID_user === req.body.ID_user ? Gasto_total : Gasto_total / Users.length;
                    const Email = "test@gmail.com";
                    const Fecha_Union = Fecha_creacion;
                    const PAGADO = ID_user === req.body.ID_user ? 'SI' : 'NO';


                    const sql_query = `INSERT INTO Gasto_Conjunto_Master(ID_user, ID_group, User, Email, Fecha_Union, Contribucion, PAGADO) 
                                        VALUES ('${ID_user}', '${newId}', '${ID_user}', '${Email}', '${Fecha_Union}', '${Contribucion}', '${PAGADO}')`;
      ...
      app.get('/GastoConjuntoPorGrupo/:IDGrupoGasto', (req, res) => {
        const IDGrupoGasto = req.params.IDGrupoGasto;
      
        // Consulta para obtener todos los datos de la tabla Gasto_Conjunto
        const query = `SELECT * FROM Gasto_Conjunto WHERE IDGrupoGasto = ?`;
      
        conexion.query(query, [IDGrupoGasto], (error, results) => {
          if (error) {
            console.error(error);
            res.status(500).send('Error al obtener datos del grupo de gasto.');
          } else {
            res.json(results);
          }
        });
...
const params = idUsuarios;
      ...
      app.get('/verificarNombreUsuario/:nombre', (req, res) => {
  const nombreUsuario = req.params.nombre;

  // Realiza una consulta SQL para verificar si el nombre de usuario existe en la tabla inicio_sesion
  const selectQuery = 'SELECT COUNT(*) AS count FROM Inicio_sesion WHERE Usuario = ?';
  const values = [nombreUsuario];

  conexion.query(selectQuery, values, (error, results) => {
    if (error) {
      console.error('Error al consultar la base de datos:', error);
      res.status(500).json({ existe: false }); // Devuelve false en caso de error
    } else {
      // Verifica el recuento de filas
    ...
    app.post('/insertarGrupo', (req, res) => {
        const { NombreGrupo, DescripcionGrupo, FechaCreacion, IDCreador } = req.body;
      
        const insertQuery = 'INSERT INTO Grupos (NombreGrupo, DescripcionGrupo, FechaCreacion, IDCreador) VALUES (?,  ?, ?, ?)';
        const values = [NombreGrupo, DescripcionGrupo, FechaCreacion, IDCreador];
        ...
        conexion.query(insertQuery, values, (error, results) => {
          if (error) {
            ...
            // Obtén el ID recién insertado
            const IDGrupo = results.insertId;
      
            console.log('Inserción exitosa en la base de datos. IDGrupo:', IDGrupo);
      
            // Devuelve el IDGrupo en la respuesta JSON
            res.status(200).json({ message: 'Inserción exitosa en la base de datos', IDGrupo });
          }
        });
      });
      
      app.post('/agregarEntrada', (req, res) => {
        // Obtener los valores del usuario y el IDGrupoGasto desde el cuerpo de la solicitud
        const { ID_User, IDGrupoGasto } = req.body;
    
        // Verificar que ambos valores estén presentes en la solicitud
        if (!ID_User || !IDGrupoGasto) {
            return res.status(400).json({ error: 'Faltan datos requeridos' });
        }
    
        // Verificar si ya existe una entrada con el mismo ID_User e IDGrupoGasto
        const checkDuplicateSql = 'SELECT * FROM grupos_usuarios WHERE ID_User = ? AND IDGrupoGasto = ?';
        const checkDuplicateValues = [ID_User, IDGrupoGasto];
    
        conexion.query(checkDuplicateSql, checkDuplicateValues, (error, results) => {
            if (error) {
            ...
            if (results.length > 0) {
                // Ya existe una entrada con el mismo ID_User e IDGrupoGasto
                return res.json({ mensaje: 'El usuario ya está en el gasto' });
            }
    
            // No hay duplicados, proceder con la inserción
            const insertSql = 'INSERT INTO grupos_usuarios (ID_User, IDGrupoGasto) VALUES (?, ?)';
            const insertValues = [ID_User, IDGrupoGasto];
    
            conexion.query(insertSql, insertValues, (insertError, result) => {
                if (insertError) {
                    console.error('Error al insertar entrada:', insertError);
                    return res.status(500).json({ error: 'Error en la base de datos' });
                }
    
                console.log('Entrada insertada con éxito:', result);
                res.json({ mensaje: 'Entrada insertada con éxito' });
            });
        });
    });
    ...
    app.post('/saldoPendiente', (req, res) => {
        const idGrupoGasto = req.body.IDGrupoGasto;
    
        // Obtener los ID_group de la tabla Gasto_Conjunto que coincidan con el IDGrupoGasto
        const sqlIDGroup = `
            SELECT ID_group
            FROM Gasto_Conjunto
            WHERE IDGrupoGasto = ?;
        ...
        conexion.query(sqlIDGroup, [idGrupoGasto], (error, resultIDGroup) => {
            if (error) {
                res.status(500).json({ error: 'Error al obtener los ID_group' });
            } else {
                const idGroups = resultIDGroup.map(row => row.ID_group);
    

                const sqlGastosCreadores = `
                SELECT gc.ID_group, gc.ID_user AS Creator, gc.Gasto_total
                FROM Gasto_Conjunto gc
                WHERE gc.ID_group IN (?);
            `;
            
            conexion.query(sqlGastosCreadores, [idGroups], (error, resultGastosCreadores) => {
                if (error) {
                    console.error('Error al obtener la información de los creadores de gastos:', error);
                } else {

                    // Obtener las filas de Gasto_Conjunto_Master que coinciden con los ID_group
                    const sqlFilas = `
                    SELECT ID_group, User, Contribucion, PAGADO
                    FROM Gasto_Conjunto_Master
                    WHERE ID_group IN (?);
                `;
                
                conexion.query(sqlFilas, [idGroups], (error, resultFilas) => {
                    if (error) {
                        res.status(500).json({ error: 'Error al obtener las filas' });
                    } else {
                        // Crear un objeto para almacenar la información de deudas y pagos
                        const tablaDeudasPagos = {};
                
                        // Procesar cada fila y calcular deudas y pagos
                        resultFilas.forEach(row => {
                            const { ID_group, User, Contribucion, PAGADO } = row;
                
                            // Inicializar el objeto para el grupo si no existe
                            if (!tablaDeudasPagos[ID_group]) {
                                tablaDeudasPagos[ID_group] = {};
                            }
                
                            // Inicializar la entrada para el usuario si no existe
                            if (!tablaDeudasPagos[ID_group][User]) {
                                tablaDeudasPagos[ID_group][User] = { pagado: 0, debe: 0 };
                            }
                
                            // Actualizar la entrada según si está pagado o no
                            if (PAGADO !== 'SI') {
                                tablaDeudasPagos[ID_group][User].debe = Contribucion;
                            }
                        });

                        const deudasPorGasto = {};
                        resultGastosCreadores.forEach(gasto => {
                            const { ID_group, Creator, Gasto_total } = gasto;
                        
                            // Obtener los usuarios que le deben dinero al pagador (Creator)
                            const usuariosDeben = Object.keys(tablaDeudasPagos[ID_group])
                                .filter(usuario => usuario !== Creator && tablaDeudasPagos[ID_group][usuario].debe > 0);
                        
                            // Inicializar el objeto de deudas para este gasto
                            deudasPorGasto[ID_group] = {
                                pagador: Creator,
                                deben: usuariosDeben.map(usuario => ({
                                    nombre: usuario,
                                    deuda: (tablaDeudasPagos[ID_group][usuario].debe / Gasto_total) * Gasto_total,
                                })),
                            };
                        });
                        
                        
                        const pagosCalculados = calcularDeudas(deudasPorGasto);

                        console.log(pagosCalculados);
                        const deudasConCancelacion = cancelarDeudasMutuas(pagosCalculados);
                        console.log(deudasConCancelacion);
                       // res.json(deudasPorGasto);
                        res.json(deudasPorGasto);

                    }
                });
                        
                    //res.json(resultGastosCreadores);
                }
            });
    ...
    function calcularDeudas(deudasPorGasto) {
        const deudas = {};
    
        // Iterar sobre cada gasto en deudasPorGasto
        Object.keys(deudasPorGasto).forEach(idGasto => {
            const { pagador, deben } = deudasPorGasto[idGasto];
    
            // Inicializar las entradas en el objeto deudas para el pagador
            if (!deudas[pagador]) {
                deudas[pagador] = { deuda: {}, deben: {} };
            }
    
            // Iterar sobre cada deudor en el gasto
            deben.forEach(deudor => {
                const { nombre, deuda } = deudor;
    
                // Actualizar la propiedad "deuda" para el pagador
                deudas[pagador].deuda[nombre] = (deudas[pagador].deuda[nombre] || 0) + deuda;
    
                // Inicializar la entrada en la propiedad "deben" para el deudor
                if (!deudas[nombre]) {
                    deudas[nombre] = { deuda: {}, deben: {} };
                }
    
                // Actualizar la propiedad "deben" para el deudor
                deudas[nombre].deben[pagador] = (deudas[nombre].deben[pagador] || 0) + deuda;
            });
        });
    ...
    function cancelarDeudasMutuas(deudasCalculadas) {
        // Iterar sobre cada usuario
        Object.keys(deudasCalculadas).forEach(usuario => {
            const { deuda, deben } = deudasCalculadas[usuario];
    
            // Iterar sobre las deudas del usuario
            Object.keys(deuda).forEach(otroUsuario => {
                // Verificar si el otro usuario también tiene una deuda hacia el usuario actual
                if (deben[otroUsuario]) {
                    const deudaMutua = Math.min(deuda[otroUsuario], deben[otroUsuario]);
    
                    // Cancelar la deuda mutua
                    deuda[otroUsuario] -= deudaMutua;
                    deben[otroUsuario] -= deudaMutua;
                }
            });
        });
    
        return deudasCalculadas;
    }
    ...
    app.get('/obtenerGrupo/:idGrupo', (req, res) => {
        const idGrupo = req.params.idGrupo;
    
        const selectQuery = 'SELECT * FROM Grupos WHERE IDGrupoGasto = ?';
        ...
        conexion.query(selectQuery, values, (error, results) => {
            if (error) {
                ...
                if (results.length > 0) {
                    ...
                    console.log('Consulta exitosa en la base de datos. Grupo:', grupo);
                    res.status(200).json({ message: 'Consulta exitosa en la base de datos', grupo });
                } else {
    ...
    app.get('/gruposUsuario/:ID_User', (req, res) => {
        const ID_User = req.params.ID_User;
    
        const selectQuery = 'SELECT IDGrupoGasto FROM grupos_usuarios WHERE ID_User = ?';
        ...
        conexion.query(selectQuery, values, (error, results) => {
            if (error) {
                ...
                const IDGrupoGastos = results.map(result => result.IDGrupoGasto);
                console.log('Consulta exitosa en la base de datos. IDGrupoGastos:', IDGrupoGastos);
                res.status(200).json({ message: 'Consulta exitosa en la base de datos', IDGrupoGastos });
            }
        });
    });


    app.get('/usuariosGrupo/:IDGrupoGasto', (req, res) => {
        const IDGrupoGasto = req.params.IDGrupoGasto;
    
        const selectQuery = 'SELECT ID_User FROM grupos_usuarios WHERE IDGrupoGasto = ?';
        const values = [IDGrupoGasto];
        ...
        conexion.query(selectQuery, values, (error, results) => {
            if (error) {
                ...
                const IDUsuarios = results.map(result => result.ID_User);
                console.log('Consulta exitosa en la base de datos. ID de Usuarios en el grupo:', IDUsuarios);
                res.status(200).json({ message: 'Consulta exitosa en la base de datos', IDUsuarios });
            }
        });
    });
